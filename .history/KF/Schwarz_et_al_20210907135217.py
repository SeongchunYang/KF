...
class WindowUnscentedKalmanFilter(object):
    '''
    ...
    This paper has the similar problem.
    '''
    def __init__(
        self, 
        dim_z, 
    ...
    def predict(self, **kwargs):
        ...
        self.compute_process_sigmas(self.fx, z = self.z_updated, P = self.P_updated, **kwargs)
        # pass sigma points through unscented transform to compute P(Z_{t+1})
        self.z, self.P = WindowUnscentedKalmanFilter.UT(
    ...
    def smoother(self, zs, Ps, Qs = None, **kwargs):
        n, dim_z = zs.shape
        ...
        # smoother gain
        # denoted S to differentiate from filter gain
        Ss = np.zeros((n, dim_z, dim_z))
        
        num_sigmas = self._num_sigmas

        z, P, Pp = zs.copy(), Ps.copy(), Ps.copy()
        sigmas_f = np.zeros((num_sigmas, dim_z))

        for t in reversed(range(n-1)):
            # create sigma points from state estimates at t, pass through state transition fn
            # (equivalent to 'compute_process_sigma' above)
            sigmas = self.points_fn.sigma_points(zs[t], Ps[t])
            for i in range(num_sigmas):
                sigmas_f[i] = self.fx(sigmas[i], **kwargs)

            # pass sigma points through unscented transform to compute P(Z_{t+1})
            # (NOT the conditional P(Z_{t+1}|Z_t)
            zp, Pp[t] = UnscentedKalmanFilter.UT(
                sigmas = sigmas_f,
                Wm  = self.Wm,
                Wc = self.Wc,
                noise_cov = self.Q
            ...
            # compute cross covariance (Cov(Z_{t+1}, Z_t))
            Pzz = 0
            for i in range(num_sigmas):
                _zp = np.subtract(sigmas_f[i], zp)
                _z  = np.subtract(sigmas[i], zs[t])
                Pzz += self.Wc[i] * np.outer(_z, _zp)

            # compute gain
            # Var[Z_{t+1}] = Pp
            # Cov(Z_t,Z_{t+1}) = Pzz
            # S = Cov/Var
            S = dot(Pzz, np.linalg.inv(Pp[t]))

            # update
            Ss[t] = S
            z[t] += dot(S, zs[t+1] - zp)
            P[t] += dot(S, Ps[t+1] - Pp[t]).dot(S.T)
