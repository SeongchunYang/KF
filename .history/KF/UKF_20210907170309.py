...
class UnscentedKalmanFilter(object):
    ...
    def compute_log_likelihood(self,S,innovation):
        self.mahalanobis = self.innovation.reshape(-1,1).T @ self.IPxx_c_p @ self.innovation.reshape(-1,1)
        self.log_likelihood = -1/2 * (np.log(np.linalg.det(self.Pxx_c_p)) + 2 * np.log(2 * np.pi) + self.mahalanobis)