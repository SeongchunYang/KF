...
# Copyright (c) 2015 Roger R. Labbe Jr
    ...
    # Differences between this and the original are as follows.
        ...
        # Direct integration of fading_memory function. If one want to be rid of this, simply remove them from UT.
# 2.
...
class UnscentedKalmanFilter(object):
    '''
    Should one wish, fading memory is built in.
    Simply initialize with option with fading_memory set to True
    and provide a alpha_sq parameter.
    Parameters
    ----------
    dim_z       :   int
    ...
    fading_memory:  bool
        whether to use fading memory or not
    alpha_sq    :   float
        fading memory filter parameter
    '''
    def __init__(self, dim_z, dim_x, z0, P0, fx, hx, points_fn, Q, R, fading_memory = None, alpha_sq = None):
        ...
        self.fading_memory  =   fading_memory or False
        self.alpha_sq       =   alpha_sq or 1.01

    def UT(self, sigmas, Wm, Wc, noise_cov):
        kmax, n = sigmas.shape
        z = dot(Wm, sigmas)
        residual = sigmas - z[np.newaxis,:]
        if self.fading_memory:
            _P = dot(residual.T, dot(np.diag(Wc), residual))
            _P = self.options['alpha_sq'] * _P
            P = _P + noise_cov
        else:
